...
- **Custom Image Management** – Manage your uploads effortlessly. Simply hover over any custom image in the pool or tier to **rename** it using the pencil icon, or delete it from your pool.
...
- **Backup & Restore** – Save your complete tier list state as a JSON file. Perfect for moving between devices or keeping long-term backups of your rankings.
  > **Note:** Unlike Share Link, JSON export/import preserves ALL items including local/custom imported images and their custom names.
...
- **Dynamic Portability** – All exports now prioritize accuracy, ensuring anime titles (and custom image names) are used in text files and properly restored in JSON imports.
...
3. **Build Your Pool**:
   ...
   - Use the **Import** tab to add your own custom images via URL or local file upload (multi-select supported).
   - Click the **+ Add to Pool** button on any item to add it to your unranked collection.
4. **Manage Custom Items**: Hover over any custom uploaded item to see management icons. Use the **Pencil icon** to rename the image or the **X icon** (in the pool) to remove it.
5. **Rank Your Items**: Drag and drop items from the pool into the tiers. You can reorder items within tiers or swap them as needed.
6. **Customize Tiers**: Click on tier names to rename them or change their colors to match your aesthetic.
7. **Save & Share**:
   - **Save the Tierlist**: Click to download a complete JSON backup with all items and settings (including custom images and names).
